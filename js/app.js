...
<div class="detail-price ${parseHarga(p.harga) === 0 ? 'free-request' : ''}">
  ${
    ...
    ${(p.kategori === "pernikahan" || p.kategori === "lainnya") ? `
    <div class="item-tambahan">
      <b>Item Tambahan</b><br>

      <label class="item-box">
        <input type="checkbox" data-harga="0" value="Transport">
    ...
    <div class="lokasi-mode">
      <label>Ketik Lokasi</label>

    <input type="text" id="lokasiAcara" placeholder="Contoh: Gedung Serbaguna atau link shareloc">
  ...
  function hitungTotal() {
    ...
    if (backdropCheck && backdropCheck.checked) {
      total += (parseInt(meterInput.value || 0) * 20000);
    }
  ...
  if (meterInput) {
    meterInput.oninput = () => {
      ...
      hargaBackdrop.textContent = m
        ? `Harga: Rp ${(m * 20000).toLocaleString("id-ID")}`
        : "";
      hitungTotal();
    };
  }
